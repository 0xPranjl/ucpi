...
contract verio {
 ...
 function addnode(string memory organisation,bool _vote) external {
     ...
     if(nodevote[organisation].novote+nodevote[organisation].yesvote==nodes.length){
         if(nodevote[organisation].yesvote>=nodevote[_organisation].novote){
...
require(nodereplacer[organisation]==false,"organisation voting pending wait for result to reapply");
